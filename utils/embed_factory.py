...
class EmbedFactory:
    ...
    def success(title: str, description: str = None, **kwargs) -> discord.Embed:
        ...
        embed = discord.Embed(title=title, description=description, color=EmbedFactory.SUCCESS)
    ...
    def error(title: str, description: str = None, **kwargs) -> discord.Embed:
        ...
        embed = discord.Embed(
            title=f"❌ {title}", description=description, color=EmbedFactory.ERROR
        )
        return EmbedFactory._add_fields(embed, kwargs)
    ...
    def warning(title: str, description: str = None, **kwargs) -> discord.Embed:
        ...
        embed = discord.Embed(
            title=f"⚠️ {title}", description=description, color=EmbedFactory.WARNING
        )
        return EmbedFactory._add_fields(embed, kwargs)
    ...
    def info(title: str, description: str = None, **kwargs) -> discord.Embed:
        ...
        embed = discord.Embed(title=title, description=description, color=EmbedFactory.INFO)
        return EmbedFactory._add_fields(embed, kwargs)
    ...
    def signal_display(signal: Dict[str, Any], detailed: bool = False) -> discord.Embed:
        ...
        if signal.get("expiry_type"):
            embed.add_field(
                name="Expiry", value=signal["expiry_type"].replace("_", " ").title(), inline=True
            )
    ...
    def signal_added(signal_id: int, parsed_signal: Any, author: str = None) -> discord.Embed:
        ...
        embed.add_field(name="Instrument", value=parsed_signal.instrument, inline=True)
        embed.add_field(name="Direction", value=parsed_signal.direction.upper(), inline=True)
        embed.add_field(
            name="Stop Loss", value=EmbedFactory._format_price(parsed_signal.stop_loss), inline=True
        )
        ...
        embed.add_field(name="Limits", value=limits_str, inline=False)
        embed.add_field(
            name="Expiry", value=parsed_signal.expiry_type.replace("_", " ").title(), inline=True
        )
    ...
    def bot_status(stats: Dict[str, Any], bot_info: Dict[str, Any]) -> discord.Embed:
        ...
        embed = discord.Embed(title="🤖 Bot Status", color=EmbedFactory.INFO)

        # Basic info
        embed.add_field(name="Version", value=bot_info.get("version", "1.0.0"), inline=True)
        embed.add_field(name="Guilds", value=bot_info.get("guilds", 0), inline=True)
        embed.add_field(name="Latency", value=f"{bot_info.get('latency', 0)}ms", inline=True)

        # Monitoring info
        embed.add_field(
            name="Monitored Channels", value=bot_info.get("monitored_channels", 0), inline=True
        )
        embed.add_field(
            name="Database",
        ...
        # Signal statistics
        embed.add_field(name="📊 Signal Statistics", value="─" * 20, inline=False)
        embed.add_field(name="Total Signals", value=stats.get("total_signals", 0), inline=True)
        embed.add_field(
            name="Currently Tracking", value=stats.get("tracking_count", 0), inline=True
        )

        # Overall performance
        overall = stats.get("overall", {})
        if overall:
            embed.add_field(name="Win Rate", value=f"{overall.get('win_rate', 0)}%", inline=True)

        return embed

    @staticmethod
    def active_signals_list(
        signals: List[Dict[str, Any]], guild_id: int, instrument: Optional[str] = None
    ) -> discord.Embed:
        ...
        embed = discord.Embed(
            title="📊 Active Signals",
            description=f"Found {len(signals)} active signal(s)"
            + (f" for {instrument}" if instrument else ""),
