...
class _NewsManager:
    ...
    def is_alert_bot_news_active_for(self, instrument):
        # These fixtures use normal (non-dryrun) events, so the alert-bot view
        # matches the client view. The tick path only calls this one.
        return self.is_news_active_for(instrument)
